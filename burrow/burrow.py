...
def handle_token_price(price_data_list):
    ...
    for price_data in price_data_list["prices"]:
        ...
        token_price_data[price_data["asset_id"]] = {"price": p, "symbol": token_metadata["symbol"], "decimals": token_metadata["decimals"]}
...
def list_token_data():
    ...
    for assets_data in assets_data_list:
        ...
        if token_id not in token_price_data:
            continue
        total_supplied_balance = int(assets_data["supplied"]["balance"]) + int(assets_data["reserved"]) + int(assets_data["prot_fee"])
        ...
        total_burrow_balance = handler_decimal(int(assets_data["borrowed"]["balance"]) / multiply_decimals(token_decimals), 6)
        borrow_apy = "0.00"
        ...
        ret_data = {
            ...
            "supply_apy": handler_decimal(float(assets_data["supply_apr"]) * 100 + float(net_tvl_apy * net_tvl_multiplier) * 100 + float(supply_farm_apy), 2),
            "total_supplied_balance": total_supplied_balance,
            "total_supplied_price": handler_decimal(float(total_supplied_balance) * float(token_price_data[token_id]["price"]), 6),
            "total_burrow_balance": total_burrow_balance,
            "total_burrow_price": handler_decimal(float(total_burrow_balance) * float(token_price_data[token_id]["price"]), 6),
            "available_liquidity_balance": handler_decimal(available_liquidity - available_liquidity * 0.001, 6),
            "available_liquidity_price": handler_decimal((available_liquidity - available_liquidity * 0.001) * float(token_price_data[token_id]["price"]), 6)
        }
